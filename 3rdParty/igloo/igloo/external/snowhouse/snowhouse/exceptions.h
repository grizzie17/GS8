...
namespace snowhouse {
  ...
  {
  public:
    static std::unique_ptr<ExceptionType>& last_exception()
    {
      static std::unique_ptr<ExceptionType> last;
      return last;
    }
...
{ \
  ...
  { \
    ExceptionStorage<EXCEPTION_TYPE>::last_exception() = std::unique_ptr<EXCEPTION_TYPE>(new EXCEPTION_TYPE(e)); \
  } \
  catch(...) \
  { \
