...
{
public:
    SampleReadCallBack( char* s )
            : m_sData( s )
            , m_nLength( ::strlen( s ) )
    {}

    virtual size_t Read  // RTN:	returns number of bytes actually read
    ...
    {
        size_t n = nBytesToRead < m_nLength ? nBytesToRead : m_nLength;
	...
	*/
    virtual size_t GetSize  // RTN:	return file (stream) size in bytes
            ( void )
    {
        return m_nLength;
    }
	...
	*/
    ...
    virtual size_t GetName              // RTN:	string length of name
...
\+---------------------------------------------------------------------*/
CXMLSampleLoader::CXMLSampleLoader( void )
        : m_pApplication( 0 )
        , m_pVehicleData( 0 )
        , m_nMilliStart( 0 )
        , m_nMilli( 0 )
...
void
CXMLSampleLoader::SetApplication( CApplicationGaugePtr p )
{
    m_pApplication = p;
...
//BEGIN_OVERRUN_WARNING
void
CXMLSampleLoader::UpdateActivePage( void )
{
    CalculateMilliTime();
    ...
    if ( pDictEnum )
    {
        while ( pDictEnum->MoveNext() )
        {
            ...
            if ( sKey == "VEHICLE SHUTDOWN" || sKey == "Alerts Active" )
                continue;
        ...
        while ( pDictEnum->MoveNext() )
        {
            ...
            if ( pv )
            {
                ...
                case 2:  // sine
                    ...
                    break;
                case 3:  // square
                    fAmplitude = GFLOAT_0 < sinf( GFLOAT_2PI * f ) ? GFLOAT_1
                                                                   : GFLOAT_0;
                    break;
                case 4:  // triangle
                ...
                case CVariantData::T_BOOL:
                    {
                        ...
                        sprintf_s( sData, sizeof( sData ),
                                "<Param ID='%s' Value='%c'/>", sKey.Pointer(),
                                c );
                        strcat_s( m_sData, sizeof( m_sData ), sData );
                    }
                    break;
                ...
                case CVariantData::T_DATETIME:
                    {
                        ...
                        t.FormatGMT(
                                sTime, sizeof( sTime ), "%Y-%m-%d %H:%M:%S Z" );
                        sprintf_s( sData, sizeof( sData ),
                                "<Param ID='%s' Value='%s'/>", sKey.Pointer(),
                                sTime );
                        strcat_s( m_sData, sizeof( m_sData ), sData );
                    }
                    break;
                case CVariantData::T_FLOAT:
                    {
                        ...
                        sprintf_s( sData, sizeof( sData ),
                                "<Param ID='%s' Value='%f'/>", sKey.Pointer(),
                                (float)fValue );
                        strcat_s( m_sData, sizeof( m_sData ), sData );
                    }
                    break;
                case CVariantData::T_INTEGER:
                    {
                        ...
                        sprintf_s( sData, sizeof( sData ),
                                "<Param ID='%s' Value='%ld'/>", sKey.Pointer(),
                                fValue );
                        strcat_s( m_sData, sizeof( m_sData ), sData );
                    }
                    break;
                case CVariantData::T_STRING:
                    {
                        if ( pvHigh && pvLow )
                        {
                            ...
                            long fMagnitude = fHigh - fLow;
                            long fValue
                                    = long( fMagnitude * fAmplitude ) + fLow;
                            sprintf_s( sData, sizeof( sData ),
                                    "<Param ID='%s' Value='%ld'/>",
                                    sKey.Pointer(), fValue );
                            strcat_s( m_sData, sizeof( m_sData ), sData );
...
void
CXMLSampleLoader::CalculateMilliTime( void )
{
    ::ftime( &m_tTimeb );
