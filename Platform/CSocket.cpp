...
static void
fillAddr(
        ConstCCharStringRef rAddress, unsigned short nPort, sockaddr_in& addr )
{
...
\+---------------------------------------------------------------------*/
CSocket::CSocket( SOCKETHDL nSockDesc )
        : m_nSockDesc( nSockDesc )
        , m_nError( 0 )
{}

CSocket::CSocket( const CSocket&  //r
        )
        : m_nSockDesc( 0 )
        , m_nError( 0 )
{}
...
CSocket::CSocket( int type, int protocol )
        : m_nSockDesc( 0 )
        , m_nError( 0 )
{
#if defined( OS_MSWIN )
...
\+---------------------------------------------------------------------*/
CCharString
CSocket::GetLocalAddress( void )
{
    sockaddr_in addr;
    size_t      nAddrLen = sizeof( addr );
    CCharString s;

    if ( getsockname( m_nSockDesc, (sockaddr*)&addr, (socklen_t*)&nAddrLen )
            < 0 )
    {
...
\+---------------------------------------------------------------------*/
void
CSocket::SetLocalPort( unsigned short n )
{
    sockaddr_in tAddr;
...
\+---------------------------------------------------------------------*/
void
CSocket::SetLocalAddressAndPort( ConstCCharStringRef rAddress,
...
\+---------------------------------------------------------------------*/
unsigned short
CSocket::ResolveService( ConstCCharStringRef rService,
...
\+---------------------------------------------------------------------*/
CSocketCommunicate::CSocketCommunicate( int type, int protocol )
...
CSocketCommunicate::CSocketCommunicate( SOCKETHDL nNewConnSD )
...
CSocketCommunicate::~CSocketCommunicate( void )
{}
...
\+---------------------------------------------------------------------*/
void
CSocketCommunicate::Connect(
        ConstCCharStringRef sForeignAddress, unsigned short nForeignPort )
{
    ...
    if ( ::connect( m_nSockDesc, (sockaddr*)&tDestAddr, sizeof( tDestAddr ) )
            < 0 )
    {
...
\+---------------------------------------------------------------------*/
void
CSocketCommunicate::Send( const void* pBuffer, size_t nBufferLength )
{
...
\+---------------------------------------------------------------------*/
int
CSocketCommunicate::Recv( void* pBuffer, size_t nBufferLength )
{
    int nResult;
...
CCharStringRef
CSocketCommunicate::GetForeignAddress( void )
{
    return m_sForeignAddress;
}

unsigned short
CSocketCommunicate::GetForeignPort( void )
{
...
CSocketTCP::CSocketTCP( SOCKETHDL nNewConnSD )
        : inherited( nNewConnSD )
{}
...
\+---------------------------------------------------------------------*/
CSocketTCPServer::CSocketTCPServer( unsigned short nLocalPort,
        int                                        nQueueLen  //= 5
        )
        : inherited( SOCK_STREAM, IPPROTO_TCP )
{
...
CSocketTCPServer::CSocketTCPServer(
        ConstCCharStringRef rLocalAddress, unsigned short nLocalPort,
        int nQueueLen  //= 5
        )
        : inherited( SOCK_STREAM, IPPROTO_TCP )
{
...
\+---------------------------------------------------------------------*/
void
CSocketTCPServer::SetListen( int nQueueLen )
{
...
CSocketUDP::CSocketUDP( unsigned short nLocalPort,
...
\+---------------------------------------------------------------------*/
void
CSocketUDP::SetBroadcast( void )
{
    if ( m_nSockDesc )
    {
        int nPermission = 1;
        if ( setsockopt( m_nSockDesc, SOL_SOCKET, SO_BROADCAST,
                     (raw_type*)&nPermission, sizeof( nPermission ) )
                < 0 )
        {
            InternalError();
        }
...
\+---------------------------------------------------------------------*/
void
CSocketUDP::SetSharedPort( void )
{
    if ( m_nSockDesc )
    {
        unsigned int opt = 1;
        if ( setsockopt( m_nSockDesc, SOL_SOCKET, SO_REUSEADDR, (raw_type*)&opt,
                     sizeof( opt ) )
                < 0 )
        {
            InternalError();
...
#    define NOSUPPORT EAFNOSUPPORT
    ...
    // Try to disconnect
    if ( ::connect( m_nSockDesc, (sockaddr*)&tNullAddr, sizeof( tNullAddr ) )
            < 0 )
    {
...
\+---------------------------------------------------------------------*/
void
CSocketUDP::SendTo( const void* pBuffer, int nBufferLen,
        ConstCCharStringRef rForeignAddress, unsigned short nForeignPort )
{
    sockaddr_in tDestAddr;
    ...
    if ( nBufferLen
            != sendto( m_nSockDesc, (raw_type*)pBuffer, nBufferLen, 0,
...
\+---------------------------------------------------------------------*/
int
CSocketUDP::RecvFrom( void* pBuffer, size_t nBufferLen,
        CCharStringPtr psSourceAddress, unsigned short* pnSourcePort )
{
    int         nResult = 0;
...
\+---------------------------------------------------------------------*/
void
CSocketUDP::SetMulticastTTL( unsigned char cMulticastTTL )
{
    if ( setsockopt( m_nSockDesc, IPPROTO_IP, IP_MULTICAST_TTL,
                 (raw_type*)&cMulticastTTL, sizeof( cMulticastTTL ) )
            < 0 )
    {
...
\+---------------------------------------------------------------------*/
void
CSocketUDP::JoinGroup( ConstCCharStringRef rMulticastGroup )
{
    struct ip_mreq mr;
    ...
    int sock = socket(
            AF_INET, SOCK_DGRAM, IPPROTO_UDP );  // temporary socket variable
    if ( sock )  // just to get local host address
    {
        if ( -1 < ioctl( sock, SIOCGIFADDR, &ifr ) )
        {
            mr.imr_interface.s_addr
                    = ( (struct sockaddr_in*)&ifr.ifr_ifru.ifru_addr )
                              ->sin_addr.s_addr;
    ...
    if ( setsockopt( m_nSockDesc, IPPROTO_IP, IP_ADD_MEMBERSHIP, (raw_type*)&mr,
                 sizeof( mr ) )
            < 0 )
    {
        LOCAL_PRINT(
                "Multicast group (%s) join failed (setsockopt())\n", sTemp );
        InternalError();
    }
}
...
\+---------------------------------------------------------------------*/
void
CSocketUDP::LeaveGroup( ConstCCharStringRef rMulticastGroup )
{
    struct ip_mreq multicastRequest;

    multicastRequest.imr_multiaddr.s_addr
            = inet_addr( rMulticastGroup.Pointer() );
