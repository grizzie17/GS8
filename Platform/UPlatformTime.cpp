/*---------------------------------------------------------------------+\
|
|	UPlatformTime.cpp  --  brief description of what UPlatformTime.c is for
|
|	Purpose:
...
static LONGLONG	g_nMilliStart = 0;
//static LONGLONG	g_nTimeFrequency = 0;
#	if defined( OS_MSWIN_WCE )
static struct tm st_tm;
#	endif
#else
static struct timeb	g_tTimeb;
static long		g_nMilliStart = 0;
#endif
		...
		)
{
	...
	struct tm	tTm;
	::ftime( &tTimeb );
	if ( 0 == gmtime_s( &tTm, &g_tTimeb.time ) )
	{
		//m_tTm = *::gmtime( &tTimeb.time );

		long	nCount = tTimeb.millitm + long(tTimeb.time & 0xFFFFF) * 1000;
		g_nMilliStart = nCount;
	}
	else
	{
		g_nMilliStart = 0;
	...
	g_nMilliStart = GetTickCount();

	//LARGE_INTEGER  frq;
	//LARGE_INTEGER  tick;

	//QueryPerformanceFrequency(&frq);
	//g_nTimeFrequency = frq.QuadPart / 1000;
	//QueryPerformanceCounter(&tick);
	//g_nMilliStart = tick.QuadPart;
#endif
		...
		)
{
	...
	struct tm	tTm;
	::ftime( &tTimeb );
	if ( 0 == gmtime_s( &tTm, &tTimeb.time ) )
	{
		//m_tTm = *::gmtime( &m_tTimeb.time );

		long	nCount = (long)(tTimeb.millitm + (tTimeb.time & 0xFFFFF) * 1000);
		long	nSpan = nCount - g_nMilliStart;
		if ( nSpan < 0 )
			nSpan += 0x100000 * 1000;
		return nSpan;
	}
	...
	{
		return g_nMilliStart;
	}
#elif defined(TIME_WINDOWS)

	return long(GetTickCount() - g_nMilliStart);
	//LARGE_INTEGER  tick;

	//QueryPerformanceCounter(&tick);
	//long nSpan = long(((tick.QuadPart - g_nMilliStart) / g_nTimeFrequency) & 0x0FFFFFFF);
	//return nSpan;
#endif
}
...
struct tm* localtime(const time_t* clock)
{
	...
	ftUtc.dwLowDateTime = (DWORD)*clock;
...
struct tm* gmtime(const time_t* clock)
{
	...
	ftLocal.dwLowDateTime = (DWORD)*clock;
			...
			case 's':
				{
					...
					tm = *t;
					mkt = mktime(&tm);
					BEGIN_IGNORE_WARNING( 4127 )	// constant expression
		...
		)
{
	...
	if (0 == (result = (atmp->tm_year - btmp->tm_year))
		&&	0 == (result = (atmp->tm_mon - btmp->tm_mon))
		&&	0 == (result = (atmp->tm_mday - btmp->tm_mday))
		&&	0 == (result = (atmp->tm_hour - btmp->tm_hour))
		&&	0 == (result = (atmp->tm_min - btmp->tm_min))
		)
