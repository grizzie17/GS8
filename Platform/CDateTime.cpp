...
// time_t CDateTime::g_nBadTimeValue = -1;
...
time_t
time_tFromSystemTime( SYSTEMTIME* pSystemTime )
{
...
void
Time_tToSystemTime( time_t dosTime, SYSTEMTIME* systemTime )
{
    LARGE_INTEGER jan1970FT = { 0 };
...
CDateTime::CDateTime(
        int nYear, int nMon, int nDay, int nHour, int nMin, int nSec )
        : m_t( 0 )
{
    Load( nYear, nMon, nDay, nHour, nMin, nSec );
}

CDateTime::CDateTime( const char* s )
{
    Parse( s );
}

CDateTime::CDateTime( ConstCCharDescriptorRef r )
{
    Parse( r );
}
...
\+---------------------------------------------------------------------*/
CDateTime::CDateTime( ConstCDateTimeRef r )
        : m_t( r.m_t )
{}
...
\+---------------------------------------------------------------------*/
bool
CDateTime::Parse( ConstCCharDescriptorRef r )
{
    bool bResult = false;
    long nYear = 0;
    long nMonth = 0;
...
long
CDateTime::ParseDate(
        const char* s, const char* sEnd, long* pYear, long* pMonth, long* pDay )
{
    long        nResult = 0;
    long        nYear = 0;
    long        nMonth = 0;
...
\+---------------------------------------------------------------------*/
long
CDateTime::ParseTime( const char* s, const char* sEnd, long* pHour,
        long* pMinute, long* pSecond )
{
    long        nResult = 0;
    long        nHour = 0;
    long        nMinute = 0;
...
\+---------------------------------------------------------------------*/
long
CDateTime::ParseFracSeconds( const char* s, const char* sEnd )
{
    long        nResult = 0;
    long        nFrac = 0;
    long        n = 0;
...
\+---------------------------------------------------------------------*/
long
CDateTime::ParseZone( const char* s, const char* sEnd, long* pSecondsOffset )
{
    long        nResult = 0;
    long        nOffset = 0;
    long        nDST = 0;
...
\+---------------------------------------------------------------------*/
bool
CDateTime::Load( int nYear, int nMon, int nDay, int nHour, int nMin, int nSec )
{
    bool bResult = false;
#if defined( OS_MSWIN )
    ...
    if ( -1 < nHour && nHour < 24 && -1 < nMin && nMin < 60 && -1 < nSec
            && nSec < 60 && 0 < nDay && nDay < 32 && 0 < nMon && nMon < 13
            && 1970 < nYear )
    {
        struct tm t = { 0 };
...
\+---------------------------------------------------------------------*/
void
CDateTime::FormatLocal( char* sBuffer, size_t nBufSize, const char* sFormat )
{
    struct tm tTm = { 0 };
...
\+---------------------------------------------------------------------*/
void
CDateTime::FormatGMT( char* sBuffer, size_t nBufSize, const char* sFormat )
{
    struct tm tTm = { 0 };
...
void
CDateTime::FormatISO( char* sBuffer, size_t nBufSize, bool bLocal )
{
    ...
    if ( bLocal && 0 != nOffset )
    {
        ...
        sprintf_s( sBuffer, nBufSize, "%s %c%02ld:%02ld%s", sTemp, cSign, nHour,
                nMinute, sDST );
    }
    else
    {
...
\+---------------------------------------------------------------------*/
long
CDateTime::TimeAdjustOffset( void )
{
    long nOffset = TimezoneOffset();
...
void
CDateTime::FormatTM(
        char* sBuffer, size_t nBufSize, const char* sFormat, struct tm* pTm )
{
...
\+---------------------------------------------------------------------*/
long
CDateTime::SpanNumber( const char* s, const char* sEnd, long* pNumber )
{
    size_t nResult = 0;
    long   nValue = 0;
...
\+---------------------------------------------------------------------*/
long
CDateTime::SpanBlanks( const char* s, const char* sEnd )
{
    const char* sStart = s;
    while ( s < sEnd )
    {
