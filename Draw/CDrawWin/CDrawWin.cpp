/*---------------------------------------------------------------------+\
|
|	CDrawWin.cpp  --  brief description of what CDrawWin.cpp is for
|
|	Purpose:
|
...
PLUGIN_CREATE_FACTORY( CDrawWin );
...
\+---------------------------------------------------------------------*/
void	CDrawWin::SetDoubleBackingstore
		(
		bool	b
		)
{
...
\+---------------------------------------------------------------------*/
void	CDrawWin::SetDisplayDirtyRectangle
		(
		bool	b
		)
{
		...
		) const
{
	...
	pt.x = pHM->cx;
	pt.y = pHM->cy;
	if ( nMapMode < MM_ISOTROPIC  &&  MM_TEXT != nMapMode )
	{
	...
	else
	{
		...
		cxPerInch = ::GetDeviceCaps( m_hdc, LOGPIXELSX );
		cyPerInch = ::GetDeviceCaps( m_hdc, LOGPIXELSY );

		pt.x = MulDiv( pt.x, cxPerInch, HIMETRIC_INCH );
		pt.y = MulDiv( pt.y, cyPerInch, HIMETRIC_INCH );
	}
	::DPtoLP( m_hdc, &pt, 1 );
	pPX->cx = pt.x;
	pPX->cy = pt.y;
...
\+---------------------------------------------------------------------*/
void*	CDrawWin::FindInternalInterface
		(
		ConstIXIDRef	rIID
		)
{
...
\+---------------------------------------------------------------------*/
bool	CDrawWin::PushCropRectangle
		(
		long	nLeft,
		long	nTop,
		long	nRight,
...
\+---------------------------------------------------------------------*/
bool	CDrawWin::RefreshRectangle
		(
		void
		)
{
...
\+---------------------------------------------------------------------*/
bool	CDrawWin::SetUpdateRectangle
		(
		long	nLeft,
		long	nTop,
		long	nRight,
...
\+---------------------------------------------------------------------*/
bool	CDrawWin::GetUpdateRectangle
		(
		long*	pLeft,
		long*	pTop,
		long*	pRight,
...
#if defined( OS_MSWIN_WCE )
			::ExtTextOut( m_hdc, int(tx+1), int(ty+1), 0, NULL, wsString, (UINT)nCount, NULL );
#else
			::ExtTextOutA( m_hdc, int(tx+1), int(ty+1), 0, NULL, sString, (UINT)nCount, NULL );
#endif
...
#if defined( OS_MSWIN_WCE )
		::ExtTextOut( m_hdc, int(tx), int(ty), 0, NULL, wsString, (UINT)nCount, NULL );
#else
		::ExtTextOutA( m_hdc, int(tx), int(ty), 0, NULL, sString, (UINT)nCount, NULL );
#endif
		...
		)
{
	...
	do_sequence
	{
		...
		hr = OleLoadPicture( pStm, (LONG)dwFileSize, FALSE, IID_IPicture, (LPVOID*)&pPicture );
		pStm->Release();
